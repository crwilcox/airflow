...
class GCPVideoIntelligenceHelper(LoggingCommandExecutor):
    ...
    def create_bucket(self):
        ...
        with NamedTemporaryFile(suffix=".mp4") as file:
            self.execute_cmd(["curl", "-s", GCP_VIDEO_SOURCE_URL, "-o", file.name])
            self.execute_cmd(["gsutil", "cp", file.name, "gs://%s/video.mp4" % GCP_BUCKET_NAME])
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Create or delete bucket with test file for system tests.')
    parser.add_argument('--action', dest='action', required=True,
                        choices=('create-bucket', 'delete-bucket',
