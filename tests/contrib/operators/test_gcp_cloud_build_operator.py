...
class GcpCloudBuildCreateBuildOperatorTestCase(TestCase):
    ...
    def test_storage_source_replace(self, hock_mock):
        ...
        op = CloudBuildCreateBuildOperator(body=current_body, project_id=TEST_PROJECT_ID, task_id="task-id")
        op.execute({})
        ...
        hock_mock.create_build(body=expected_result, project_id=TEST_PROJECT_ID)

    @mock.patch(
        "airflow.contrib.operators.gcp_cloud_build_operator.CloudBuildHook",
        **{"return_value.create_build.return_value": TEST_CREATE_BODY}
    )
    def test_repo_source_replace(self, hock_mock):
        current_body = {
